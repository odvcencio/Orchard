...
go test ./...
make build
ORCHARD_JWT_SECRET=dev-jwt-secret-12345 ORCHARD_PORT=3900 go run ./cmd/orchard serve
```

Notes:

- `ORCHARD_JWT_SECRET` must be at least 16 characters.
- `serve` applies DB migrations on startup.
- With no DB env overrides, local runs use SQLite (`orchard.db`) and `data/repos`.
...
- Run migrations only: `go run ./cmd/orchard migrate`
