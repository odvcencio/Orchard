...
import { useIssueComments, useCreateIssueComment } from '@/lib/api/hooks';
import { useAuth } from '@/lib/auth/use-auth';
import type { IssueComment } from '@/lib/api/types';
...
export function IssueTimeline({
  ...
  const { data: comments, isLoading } = useIssueComments(owner, repo, issueNumber);
  const createComment = useCreateIssueComment();
  const returnTo = `/${owner}/${repo}/issues/${issueNumber}`;
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    createComment.mutate(
      { owner, repo, number: issueNumber, body: trimmed },
      { onSuccess: () => setBody('') },
    );
  }
      ...
      {showComposer ? (
        isAuthenticated ? (
          ...
          <AuthCallout
            ...
            description="Join the discussion by signing in to Orchard."
            returnTo={returnTo}
            actionLabel="Sign in to reply"
          />
        )
      ) : null}
    </div>
