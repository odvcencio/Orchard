...
func TestSSHBootstrapFirstKeyRegistrationFlow(t *testing.T) {
	server, _ := setupTestServerWithOptions(t, api.ServerOptions{
		BootstrapSSHToken: "bootstrap-secret",
	})
	ts := httptest.NewServer(server)
	defer ts.Close()

	authToken := registerAndGetToken(t, ts.URL, "bootstrap-user")
	signer, pubText, fingerprint := newTestSSHSigner(t)
	bootstrapToken := mintSSHBootstrapTokenForTest(t, ts.URL, authToken, 300)

	reqBody := fmt.Sprintf(`{"username":"bootstrap-user","name":"agent-1","public_key":%q,"bootstrap_token":%q}`, strings.TrimSpace(pubText), bootstrapToken)
	resp, err := http.Post(ts.URL+"/api/v1/auth/ssh/bootstrap", "application/json", bytes.NewBufferString(reqBody))
	if err != nil {
		t.Fatal(err)
	}
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("ssh bootstrap: expected 200, got %d", resp.StatusCode)
	}
	...
	if bootstrapResp.Token == "" {
		t.Fatal("expected jwt token from bootstrap response")
	}
	if bootstrapResp.User.Username != "bootstrap-user" {
		t.Fatalf("expected bootstrap-user, got %q", bootstrapResp.User.Username)
	}

	challengeReq := fmt.Sprintf(`{"username":"bootstrap-user","fingerprint":"%s"}`, fingerprint)
	resp, err = http.Post(ts.URL+"/api/v1/auth/ssh/challenge", "application/json", bytes.NewBufferString(challengeReq))
	if err != nil {
		t.Fatal(err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("ssh verify after bootstrap: expected 200, got %d", resp.StatusCode)
	}
	resp.Body.Close()
	...
	if resp.StatusCode != http.StatusConflict {
		t.Fatalf("re-bootstrap: expected 409, got %d", resp.StatusCode)
	}
	resp.Body.Close()
}

func TestSSHBootstrapRejectsInvalidToken(t *testing.T) {
	server, _ := setupTestServerWithOptions(t, api.ServerOptions{
		BootstrapSSHToken: "bootstrap-secret",
	})
	ts := httptest.NewServer(server)
	...
	_ = registerAndGetToken(t, ts.URL, "bootstrap-user")
	_, pubText, _ := newTestSSHSigner(t)

	reqBody := fmt.Sprintf(`{"username":"bootstrap-user","name":"agent-1","public_key":%q,"bootstrap_token":"wrong-token"}`, strings.TrimSpace(pubText))
	resp, err := http.Post(ts.URL+"/api/v1/auth/ssh/bootstrap", "application/json", bytes.NewBufferString(reqBody))
	if err != nil {
		t.Fatal(err)
	}
	if resp.StatusCode != http.StatusUnauthorized {
		t.Fatalf("ssh bootstrap invalid token: expected 401, got %d", resp.StatusCode)
	}
	resp.Body.Close()
}

func TestSSHBootstrapMintRequiresAuth(t *testing.T) {
	server, _ := setupTestServerWithOptions(t, api.ServerOptions{
		BootstrapSSHToken: "bootstrap-secret",
	})
	ts := httptest.NewServer(server)
	...
	req, _ := http.NewRequest(http.MethodPost, ts.URL+"/api/v1/auth/ssh/bootstrap/token", bytes.NewBufferString(`{}`))
	req.Header.Set("Content-Type", "application/json")
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
	...
	if resp.StatusCode != http.StatusUnauthorized {
		t.Fatalf("mint bootstrap token without auth: expected 401, got %d", resp.StatusCode)
	}
}

func TestSSHBootstrapTokenBoundToMintedUser(t *testing.T) {
	server, _ := setupTestServerWithOptions(t, api.ServerOptions{
		BootstrapSSHToken: "bootstrap-secret",
	})
	ts := httptest.NewServer(server)
	...
	_ = registerAndGetToken(t, ts.URL, "bob")
	_, pubText, _ := newTestSSHSigner(t)
	bootstrapToken := mintSSHBootstrapTokenForTest(t, ts.URL, aliceToken, 300)

	reqBody := fmt.Sprintf(`{"username":"bob","name":"agent-1","public_key":%q,"bootstrap_token":%q}`, strings.TrimSpace(pubText), bootstrapToken)
	resp, err := http.Post(ts.URL+"/api/v1/auth/ssh/bootstrap", "application/json", bytes.NewBufferString(reqBody))
	if err != nil {
		t.Fatal(err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusUnauthorized {
		t.Fatalf("bootstrap with token minted for another user: expected 401, got %d", resp.StatusCode)
	}
}
...
func TestListPasskeys(t *testing.T) {
	...
	token := registerAndGetToken(t, ts.URL, "passkeylist")
	user, err := db.GetUserByUsername(context.Background(), "passkeylist")
	if err != nil {
		t.Fatalf("lookup user: %v", err)
	}
	if err := db.CreateWebAuthnCredential(context.Background(), &models.WebAuthnCredential{
		UserID:       user.ID,
	...
	req, _ := http.NewRequest(http.MethodGet, ts.URL+"/api/v1/user/passkeys", nil)
	req.Header.Set("Authorization", "Bearer "+token)
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
	...
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("list passkeys: expected 200, got %d", resp.StatusCode)
	}
	var payload []struct {
		ID           int64  `json:"id"`
	...
	resp.Body.Close()
	if len(payload) != 1 {
		t.Fatalf("expected 1 passkey, got %d", len(payload))
	}
	if payload[0].CredentialID != "cred-abc-123" {
		t.Fatalf("unexpected passkey credential id %q", payload[0].CredentialID)
	}
}
...
func TestProtocolAuthForPrivateRepo(t *testing.T) {
	...
	resp, err = http.Get(ts.URL + "/graft/alice/private-repo/refs")
	if err != nil {
		t.Fatal(err)
	}
	if resp.StatusCode != http.StatusUnauthorized {
...
func TestProtocolAuthBasicAuthDisabledReturns401(t *testing.T) {
	...
	req, _ = http.NewRequest("GET", ts.URL+"/graft/alice/missing-repo/refs", nil)
	req.SetBasicAuth("alice", "anything")
	resp, err = http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
...
func TestProtocolAuthBasicTokenAllowsPrivateRepoAccess(t *testing.T) {
	server, _ := setupTestServer(t)
	ts := httptest.NewServer(server)
	defer ts.Close()
	...
	createBody := `{"name":"private-repo","description":"","private":true}`
	createReq, _ := http.NewRequest("POST", ts.URL+"/api/v1/repos", bytes.NewBufferString(createBody))
	...
	if createResp.StatusCode != http.StatusCreated {
		t.Fatalf("create private repo: expected 201, got %d", createResp.StatusCode)
	}
	...
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("git info/refs with basic token auth: expected 200, got %d", resp.StatusCode)
	}
	resp.Body.Close()
}
...
func TestProtocolAuthForbiddenForPrivateRepoNonMember(t *testing.T) {
	...
	req, _ = http.NewRequest("POST", ts.URL+"/graft/alice/private-repo/objects/batch", bytes.NewBufferString(reqBody))
...
func TestGitUploadPackAdvertisementIncludesHeadSymref(t *testing.T) {
	server, _ := setupTestServer(t)
	ts := httptest.NewServer(server)
	defer ts.Close()
	...
	token := registerAndGetToken(t, ts.URL, "alice")
	createRepo(t, ts.URL, token, "repo", false)
	_ = pushSimpleGoCommit(t, ts.URL, "alice", "repo", token)
	...
	if !strings.Contains(bodyStr, " HEAD\x00") {
		t.Fatalf("expected HEAD pseudo-ref in advertisement, got body %q", bodyStr)
	}
	if !strings.Contains(bodyStr, "symref=HEAD:refs/heads/main") {
		t.Fatalf("expected HEAD symref capability, got body %q", bodyStr)
	}
	if !strings.Contains(bodyStr, " refs/heads/main") {
		t.Fatalf("expected refs/heads/main advertisement, got body %q", bodyStr)
	}
}
...
func TestGitUploadPackPackContainsRequestedCommitHash(t *testing.T) {
	server, _ := setupTestServer(t)
	ts := httptest.NewServer(server)
	defer ts.Close()
	...
	payload := append(pktLineForTest(fmt.Sprintf("want %s ofs-delta\n", gitCommitHash)), pktFlushForTest()...)
	payload = append(payload, pktLineForTest("done\n")...)
	payload = append(payload, pktFlushForTest()...)
	...
	if len(body) < len(pktLineForTest("NAK\n")) || !bytes.HasPrefix(body, []byte(pktLineForTest("NAK\n"))) {
		t.Fatalf("expected NAK pkt-line prefix, got %q", string(body))
	}

	packData := body[len(pktLineForTest("NAK\n")):]
	objects, err := gitinterop.ParsePackfile(bytes.NewReader(packData))
	if err != nil {
		t.Fatalf("parse upload-pack payload: %v", err)
	}
	...
	for _, obj := range objects {
		var gitType string
		switch obj.Type {
		case gitinterop.OBJ_COMMIT:
			gitType = gitinterop.GitTypeCommit
		case gitinterop.OBJ_TREE:
			gitType = gitinterop.GitTypeTree
		case gitinterop.OBJ_BLOB:
			gitType = gitinterop.GitTypeBlob
		case gitinterop.OBJ_TAG:
			gitType = gitinterop.GitTypeTag
		default:
			continue
		}
		if string(gitinterop.GitHashBytes(gitType, obj.Data)) == gitCommitHash {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("upload-pack did not include requested commit hash %s", gitCommitHash)
	}
}
...
func TestReceivePackUsesSidebandForStatusWhenRequested(t *testing.T) {
	...
	expectedUnpack := append([]byte{0x01}, pktLineForTest("unpack ok\n")...)
	if !bytes.Contains(body, expectedUnpack) {
		t.Fatalf("expected side-band framed unpack status, got body %q", string(body))
	}
	expectedNgLine := fmt.Sprintf("ng %s %s\n", refName, "missing object mapping")
	expectedNg := append([]byte{0x01}, pktLineForTest(expectedNgLine)...)
...
func TestBlobEndpointReturnsHighlights(t *testing.T) {
	server, _ := setupTestServer(t)
	ts := httptest.NewServer(server)
	defer ts.Close()
	...
	resp, err = http.Get(ts.URL + "/api/v1/repos/owner/repo/blob/main/main.go")
	if err != nil {
		t.Fatal(err)
	}
	if resp.StatusCode != http.StatusOK {
	...
	if blobResp.Hash == "" {
		t.Fatal("expected blob hash in response")
	}
	if blobResp.Language != "go" {
		t.Fatalf("language = %q, want %q", blobResp.Language, "go")
	}
	if len(blobResp.Highlights) == 0 {
		t.Fatal("expected non-empty highlights")
	}
	foundKeyword := false
	for _, h := range blobResp.Highlights {
	...
	if !foundKeyword {
		t.Fatalf("expected keyword capture, got %+v", blobResp.Highlights)
	}
	if len(blobResp.Entities) == 0 {
		t.Fatal("expected non-empty entities")
	}
	if blobResp.Entities[0].Key == "" {
		t.Fatalf("expected entity key, got %+v", blobResp.Entities[0])
	}
}

func TestHighlightEndpointReturnsRanges(t *testing.T) {
	server, _ := setupTestServer(t)
	ts := httptest.NewServer(server)
	defer ts.Close()

	body := `{"filename":"snippet.go","source":"package main\n\nfunc main() {}\n"}`
	resp, err := http.Post(ts.URL+"/api/v1/highlight", "application/json", bytes.NewBufferString(body))
	if err != nil {
		t.Fatal(err)
	}
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("highlight: expected 200, got %d", resp.StatusCode)
	}
	...
	if payload.Language != "go" {
		t.Fatalf("language = %q, want go", payload.Language)
	}
...
func TestMergeGateRequiresEntityOwnerApproval(t *testing.T) {
	...
	graftOwnersHash, err := store.Objects.WriteBlob(&object.Blob{Data: []byte("func:ProcessOrder @bob\n")})
	if err != nil {
		t.Fatal(err)
	}
	baseBlobHash, err := store.Objects.WriteBlob(&object.Blob{Data: []byte("package main\n\nfunc ProcessOrder() int { return 1 }\n")})
	...
	baseTreeHash, err := store.Objects.WriteTree(&object.TreeObj{
		Entries: []object.TreeEntry{
			{Name: ".graftowners", BlobHash: graftOwnersHash},
...
func TestWebhookPingRetriesAndRedelivery(t *testing.T) {
	...
	receiver := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("X-Orchard-Event") != "ping" {
			t.Errorf("expected X-Orchard-Event ping, got %q", r.Header.Get("X-Orchard-Event"))
		}
		if r.Header.Get("X-Orchard-Delivery") == "" {
			t.Errorf("expected X-Orchard-Delivery header")
		}
...
func TestPullRequestWebhookIncludesEntityChanges(t *testing.T) {
	...
	receiver := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("X-Orchard-Event") != "pull_request" {
			w.WriteHeader(http.StatusNoContent)
			return
		}
...
func mintSSHBootstrapTokenForTest(t *testing.T, baseURL, authToken string, ttlSeconds int) string {
	t.Helper()
	reqBody := fmt.Sprintf(`{"ttl_seconds":%d}`, ttlSeconds)
	req, _ := http.NewRequest(http.MethodPost, baseURL+"/api/v1/auth/ssh/bootstrap/token", bytes.NewBufferString(reqBody))
	...
	if resp.StatusCode != http.StatusCreated {
		...
		t.Fatalf("mint bootstrap token: expected 201, got %d (%s)", resp.StatusCode, strings.TrimSpace(string(body)))
	}
	var payload struct {
	...
	if strings.TrimSpace(payload.BootstrapToken) == "" {
		t.Fatal("mint bootstrap token: missing bootstrap token in response")
	}
	return strings.TrimSpace(payload.BootstrapToken)
}
