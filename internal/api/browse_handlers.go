...
func (s *Server) handleHighlightSnippet(w http.ResponseWriter, r *http.Request) {
	if s.highlightSvc == nil {
		jsonResponse(w, http.StatusOK, service.HighlightResult{})
		return
	}
	...
	result := s.highlightSvc.Highlight(req.Filename, []byte(req.Source))
