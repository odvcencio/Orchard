...
type instanceStatsProvider interface {
	InstanceStats(ctx context.Context) (database.InstanceStats, error)
}
...
type adminHealthResponse struct {
	...
	Instance  adminHealthInstance `json:"instance"`
...
type adminHealthInstance struct {
...
func (s *Server) handleAdminHealth(w http.ResponseWriter, r *http.Request) {
	...
	if statsProvider, ok := s.db.(instanceStatsProvider); ok {
		stats, err := statsProvider.InstanceStats(r.Context())
		if err != nil {
			resp.Errors = append(resp.Errors, "instance_stats")
		} else {
			resp.Instance = adminHealthInstance{
				Users:               stats.Users,
