...
func (c *Config) ValidateServe() error {
	...
	if len(c.Auth.JWTSecret) < 16 {
		return fmt.Errorf("ORCHARD_JWT_SECRET must be at least 16 characters (current length: %d)", len(c.Auth.JWTSecret))
	}
	if c.Storage.Path == "" {
...
func applyEnv(cfg *Config) {
	if v := os.Getenv("ORCHARD_HOST"); v != "" {
		cfg.Server.Host = v
	}
	if v := os.Getenv("ORCHARD_PORT"); v != "" {
		if p, err := strconv.Atoi(v); err == nil {
			cfg.Server.Port = p
		}
	}
	if v := os.Getenv("ORCHARD_TRUSTED_PROXIES"); v != "" {
		cfg.Server.TrustedProxies = parseCSV(v)
	}
	if v := os.Getenv("ORCHARD_CORS_ALLOW_ORIGINS"); v != "" {
		cfg.Server.CORSAllowedOrigins = parseCSV(v)
	}
	if v := os.Getenv("ORCHARD_DB_DRIVER"); v != "" {
		cfg.Database.Driver = v
	}
	if v := os.Getenv("ORCHARD_DB_DSN"); v != "" {
		cfg.Database.DSN = v
	}
	if v := os.Getenv("ORCHARD_STORAGE_PATH"); v != "" {
		cfg.Storage.Path = v
	}
	if v := os.Getenv("ORCHARD_JWT_SECRET"); v != "" {
		cfg.Auth.JWTSecret = v
	}
	if v := os.Getenv("ORCHARD_ENABLE_TENANCY"); v != "" {
		if enabled, err := strconv.ParseBool(v); err == nil {
			cfg.Tenancy.Enabled = enabled
		}
	}
	if v := os.Getenv("ORCHARD_TENANCY_HEADER"); v != "" {
		cfg.Tenancy.Header = v
	}
	if v := os.Getenv("ORCHARD_DEFAULT_TENANT_ID"); v != "" {
		cfg.Tenancy.DefaultTenantID = strings.TrimSpace(v)
	}
	if v := os.Getenv("ORCHARD_RESTRICT_TO_PUBLIC_REPOS"); v != "" {
		if enabled, err := strconv.ParseBool(v); err == nil {
			cfg.Launch.RestrictToPublicRepos = enabled
		}
	}
	if v := os.Getenv("ORCHARD_MAX_PUBLIC_REPOS_PER_USER"); v != "" {
		if value, err := strconv.Atoi(v); err == nil && value >= 0 {
			cfg.Launch.MaxPublicReposPerUser = value
		}
	}
	if v := os.Getenv("ORCHARD_REQUIRE_PRIVATE_REPO_PLAN"); v != "" {
		if enabled, err := strconv.ParseBool(v); err == nil {
			cfg.Launch.RequirePrivateRepoPlan = enabled
		}
	}
	if v := os.Getenv("ORCHARD_MAX_PRIVATE_REPOS_PER_USER"); v != "" {
		if value, err := strconv.Atoi(v); err == nil && value >= 0 {
			cfg.Launch.MaxPrivateReposPerUser = value
		}
	}
	if v := os.Getenv("ORCHARD_PRIVATE_REPO_ALLOWED_USERS"); v != "" {
