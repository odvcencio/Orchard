...
func TestLoadParsesTrustedProxiesAndCORSOriginsFromEnv(t *testing.T) {
	...
	t.Setenv("ORCHARD_CORS_ALLOW_ORIGINS", " https://app.example.com, ,https://admin.example.com ")
...
func TestLoadInvalidEnvValuesDoNotOverrideDefaults(t *testing.T) {
	t.Setenv("ORCHARD_PORT", "not-an-int")
	t.Setenv("ORCHARD_ENABLE_TENANCY", "not-a-bool")
	t.Setenv("ORCHARD_RESTRICT_TO_PUBLIC_REPOS", "not-a-bool")
	t.Setenv("ORCHARD_MAX_PUBLIC_REPOS_PER_USER", "-5")
	t.Setenv("ORCHARD_REQUIRE_PRIVATE_REPO_PLAN", "not-a-bool")
	t.Setenv("ORCHARD_MAX_PRIVATE_REPOS_PER_USER", "-2")
	t.Setenv("ORCHARD_PRIVATE_REPO_ALLOWED_USERS", ",, ,")
	t.Setenv("ORCHARD_TRUSTED_PROXIES", ",, ,")
	t.Setenv("ORCHARD_CORS_ALLOW_ORIGINS", ",  ,")
...
func TestLoadParsesTenancyEnv(t *testing.T) {
	...
	t.Setenv("ORCHARD_DEFAULT_TENANT_ID", " tenant-prod ")
...
func TestValidateServe(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cfg:     Default(),
			wantErr: "ORCHARD_JWT_SECRET must be set to a non-default value",
		},
		{
			...
			wantErr: "ORCHARD_JWT_SECRET must be at least 16 characters",
		},
		{
