...
// BaseObjectResolver resolves a base git object by hash when a ref-delta
// references an object not present in the current pack stream.
type BaseObjectResolver func(baseGitHash string) (PackfileObject, bool, error)
...
func ParsePackfile(r io.Reader) ([]PackfileObject, error) {
	return ParsePackfileWithBaseResolver(r, nil)
}

// ParsePackfileWithBaseResolver reads a git packfile and returns all objects.
// If resolver is provided, it can resolve ref-delta base objects by hash when
// the base object is not included in the current pack.
func ParsePackfileWithBaseResolver(r io.Reader, resolver BaseObjectResolver) ([]PackfileObject, error) {
	...
	for i := uint32(0); i < numObjects; i++ {
		...
		case OBJ_REF_DELTA:
			...
			if base == nil && resolver != nil {
				resolvedBase, ok, err := resolver(baseHashHex)
				if err != nil {
					return nil, fmt.Errorf("object %d: resolve ref-delta base %s: %w", i, baseHashHex, err)
				}
				if ok {
					base = &resolvedBase
				}
