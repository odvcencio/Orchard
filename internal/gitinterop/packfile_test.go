...
func TestSplitPktPayloadAndCapabilitiesNullDelimited(t *testing.T) {
	line := "want deadbeef\x00side-band-64k ofs-delta"
	payload, caps := splitPktPayloadAndCapabilities(line)
	if payload != "want deadbeef" {
		t.Fatalf("payload mismatch: got %q", payload)
	}
	if !caps["side-band-64k"] || !caps["ofs-delta"] {
		t.Fatalf("expected parsed capabilities, got %+v", caps)
	}
}

func TestSplitPktPayloadAndCapabilitiesSpaceDelimitedWant(t *testing.T) {
	line := "want deadbeef side-band-64k ofs-delta thin-pack"
	payload, caps := splitPktPayloadAndCapabilities(line)
	if payload != "want deadbeef" {
		t.Fatalf("payload mismatch: got %q", payload)
	}
	if !caps["side-band-64k"] || !caps["ofs-delta"] || !caps["thin-pack"] {
		t.Fatalf("expected parsed capabilities, got %+v", caps)
	}
}
