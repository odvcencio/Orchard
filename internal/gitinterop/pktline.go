...
const (
	// Max pkt-line payload is 65516 bytes (length includes 4-byte hex prefix).
	maxPktLinePayload = 65516
	// Sideband payload includes one channel byte, so data chunk max is 65515.
	sidebandMaxChunk = maxPktLinePayload - 1
)
...
func splitPktPayloadAndCapabilities(line string) (string, map[string]bool) {
	...
	// upload-pack "want" lines carry capabilities as space-separated tokens
	// after the object id on the first line.
	fields := strings.Fields(line)
	if len(fields) >= 3 && fields[0] == "want" {
		payload = strings.Join(fields[:2], " ")
		for _, capName := range fields[2:] {
			caps[capName] = true
		}
	}
	return payload, caps
