...
func (h *SmartHTTPHandler) handleInfoRefs(w http.ResponseWriter, r *http.Request) {
	...
	gitHashesByStorageRef := make(map[string]string, len(refs))

	storageRefNames := make([]string, 0, len(refs))
	for name := range refs {
		storageRefNames = append(storageRefNames, name)
	}
	sort.Strings(storageRefNames)

	// Send refs in stable order.
	first := true
	for _, name := range storageRefNames {
		gotHash := refs[name]
		...
		gitHashesByStorageRef[name] = gitHash
	}

	if svc == "git-upload-pack" && len(gitHashesByStorageRef) > 0 {
		headStorageRef := ""
		if repoMeta, err := h.db.GetRepository(r.Context(), owner, repo); err == nil {
			branch := strings.TrimSpace(repoMeta.DefaultBranch)
			if branch != "" {
				headStorageRef = normalizeGitRefName("refs/heads/" + branch)
			}
		}
		if headStorageRef == "" {
			headStorageRef = normalizeGitRefName("refs/heads/main")
		}
		headGitHash := ""
		if hash, ok := gitHashesByStorageRef[headStorageRef]; ok {
			headGitHash = hash
		} else {
			for _, candidate := range storageRefNames {
				if strings.HasPrefix(candidate, "heads/") {
					if hash, ok := gitHashesByStorageRef[candidate]; ok {
						headStorageRef = candidate
						headGitHash = hash
						break
					}
				}
		...
		if headGitHash != "" {
			headRefName := advertiseGitRefName(headStorageRef)
			capsWithSymref := capabilities + " symref=HEAD:" + headRefName
			w.Write(pktLine(fmt.Sprintf("%s HEAD\x00%s\n", headGitHash, capsWithSymref)))
			first = false
		}
	}

	for _, name := range storageRefNames {
		gitHash, ok := gitHashesByStorageRef[name]
		if !ok {
			continue
		}
...
func (h *SmartHTTPHandler) handleReceivePack(w http.ResponseWriter, r *http.Request) {
	...
	if len(packData) > 0 {
		...
		// Resolve ref-delta bases from existing repository objects when the base
		// is not included in the incoming pack stream.
		baseObjectCache := make(map[string]PackfileObject)
		resolveBaseObject := func(baseGitHash string) (PackfileObject, bool, error) {
			if cached, ok := baseObjectCache[baseGitHash]; ok {
				return cached, true, nil
			}
			gotHash, err := h.db.GetGotHash(r.Context(), repoID, baseGitHash)
			if err != nil {
				if errors.Is(err, sql.ErrNoRows) {
					return PackfileObject{}, false, nil
				}
				return PackfileObject{}, false, err
			}
			objType, data, err := store.Objects.Read(object.Hash(gotHash))
			if err != nil {
				return PackfileObject{}, false, err
			}
			gitType := gotTypeToPackType(objType)
			gitData, err := convertGotToGitData(object.Hash(gotHash), objType, data, store.Objects, r.Context(), h.db, repoID)
			if err != nil {
				return PackfileObject{}, false, err
			}
			resolved := PackfileObject{Type: gitType, Data: gitData}
			baseObjectCache[baseGitHash] = resolved
			return resolved, true, nil
		}
		objects, err := ParsePackfileWithBaseResolver(bytes.NewReader(packData), resolveBaseObject)
		if err != nil {
...
func (h *SmartHTTPHandler) sendReceivePackResult(w http.ResponseWriter, errMsg string, updates []refUpdate, refErrors map[string]string, useSideband bool) {
	...
	if useSideband {
		// report-status expects pkt-lines after sideband demux; send an inner flush.
		_, _ = w.Write(sidebandPacket(1, pktFlush()))
	}
	w.Write(pktFlush())
}
...
func (h *SmartHTTPHandler) writeReceivePackLine(w http.ResponseWriter, line string, useSideband bool) {
	if useSideband {
		// For receive-pack, sideband channel 1 carries pkt-line framed status payloads.
		_, _ = w.Write(sidebandPacket(1, pktLine(line)))
		return
	}
...
func (h *SmartHTTPHandler) rewriteTreeWithEntities(ctx context.Context, store *gotstore.RepoStore, repoID int64, treeHash object.Hash, prefix string) (object.Hash, bool, error) {
	...
	if originalGitHash, err := h.db.GetGitHash(ctx, repoID, string(treeHash)); err == nil {
		if err := h.db.SetHashMapping(ctx, &models.HashMapping{
			RepoID:     repoID,
			GotHash:    string(newTreeHash),
			GitHash:    originalGitHash,
			ObjectType: "tree",
		}); err != nil {
			return "", false, err
		}
...
// walkGotObjects walks the Graft object graph collecting objects the client doesn't have.
...
// convertGotToGitData converts a Graft object's data to git format.
