...
const (
	...
	graftProtocolHeader     = "Graft-Protocol"
	graftCapabilitiesHeader = "Graft-Capabilities"
	graftLimitsHeader       = "Graft-Limits"

	serverProtocolVersion = "1"
	serverCapabilities    = "pack,zstd,sideband"
)

// serverLimitsValue is the Graft-Limits header value advertised by this server.
...
// Handler provides HTTP endpoints for the Graft protocol (push/pull).
type Handler struct {
...
// RegisterRoutes sets up Graft protocol routes on the given mux.
func (h *Handler) RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("GET /graft/{owner}/{repo}/refs", h.handleListRefs)
	mux.HandleFunc("POST /graft/{owner}/{repo}/objects/batch", h.handleBatchObjects)
	mux.HandleFunc("GET /graft/{owner}/{repo}/objects/{hash}", h.handleGetObject)
	mux.HandleFunc("POST /graft/{owner}/{repo}/objects", h.handlePushObjects)
	mux.HandleFunc("POST /graft/{owner}/{repo}/refs", h.handleUpdateRefs)
}
...
func (h *Handler) handleBatchObjects(w http.ResponseWriter, r *http.Request) {
	...
	acceptPack := strings.Contains(r.Header.Get("Accept"), "application/x-graft-pack")
...
func (h *Handler) handlePushObjects(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(contentType, "application/x-graft-pack") {
	...
	for i, obj := range decoded {
		if err := validatePushedObject(obj.objType, obj.data, known, store.Objects, false); err != nil {
...
func (h *Handler) handleUpdateRefs(w http.ResponseWriter, r *http.Request) {
	...
	indexedTargets := make(map[object.Hash]struct{}, len(updates))
	validatedTargets := make(map[object.Hash]struct{}, len(updates))
	...
	for _, u := range updates {
		...
		if _, done := validatedTargets[target]; !done {
			if err := validateCommitClosure(store.Objects, target); err != nil {
				http.Error(w, fmt.Sprintf("set ref %s: commit graph incomplete: %v", u.Name, err), http.StatusBadRequest)
				return
			}
			validatedTargets[target] = struct{}{}
		}
		if h.indexCommit != nil {
			if _, done := indexedTargets[target]; !done {
				if err := h.indexCommit(r.Context(), owner, repo, target); err != nil {
					http.Error(w, fmt.Sprintf("set ref %s: lineage indexing failed: %v", u.Name, err), http.StatusInternalServerError)
					return
				}
				indexedTargets[target] = struct{}{}
			}
		}
		refTargets[u.Name] = target
	}
...
func (h *Handler) setProtocolHeaders(w http.ResponseWriter) {
	w.Header().Set(graftProtocolHeader, serverProtocolVersion)
	w.Header().Set(graftCapabilitiesHeader, serverCapabilities)
	w.Header().Set(graftLimitsHeader, serverLimitsValue)
}
...
func (h *Handler) authorizeRequest(w http.ResponseWriter, r *http.Request, write bool) bool {
	...
	if status == http.StatusUnauthorized {
		w.Header().Set("WWW-Authenticate", `Basic realm="orchard"`)
	}
...
func validatePushedObject(
	objType object.ObjectType,
	data []byte,
	known map[object.Hash]object.ObjectType,
	store *object.Store,
	requireReachableRefs bool,
) error {
...
func isMissingObjectErr(err error) bool {
	...
	s := strings.ToLower(err.Error())
	return strings.Contains(s, "not exist") || strings.Contains(s, "no such file") || strings.Contains(s, "missing")
}

func validateCommitClosure(store *object.Store, root object.Hash) error {
	seenCommits := make(map[object.Hash]struct{})
	seenTrees := make(map[object.Hash]struct{})

	var walkTree func(hash object.Hash) error
	walkTree = func(hash object.Hash) error {
		if hash == "" {
			return fmt.Errorf("empty tree hash")
		}
		if _, ok := seenTrees[hash]; ok {
			return nil
		}
		seenTrees[hash] = struct{}{}

		objType, data, err := store.Read(hash)
		if err != nil {
			return err
		}
		if objType != object.TypeTree {
			return fmt.Errorf("expected tree %s, got %s", hash, objType)
		}
		tree, err := object.UnmarshalTree(data)
		if err != nil {
			return err
		...
		for _, e := range tree.Entries {
			if e.IsDir {
				if err := walkTree(e.SubtreeHash); err != nil {
					return err
				}
				continue
			}
			if e.BlobHash == "" {
				return fmt.Errorf("tree entry %q missing blob hash", e.Name)
			}
			blobType, _, err := store.Read(e.BlobHash)
			if err != nil {
				return err
			}
			if blobType != object.TypeBlob {
				return fmt.Errorf("tree entry %q expected blob %s, got %s", e.Name, e.BlobHash, blobType)
			}
		}
		return nil
	}

	var walkCommit func(hash object.Hash) error
	walkCommit = func(hash object.Hash) error {
		if hash == "" {
			return fmt.Errorf("empty commit hash")
		}
		if _, ok := seenCommits[hash]; ok {
			return nil
		}
		...
		if objType != object.TypeCommit {
			return fmt.Errorf("expected commit %s, got %s", hash, objType)
		}
		commit, err := object.UnmarshalCommit(data)
		if err != nil {
			return err
