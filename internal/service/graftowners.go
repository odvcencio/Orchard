...
func parseGraftOwners(data []byte) (*graftOwnersConfig, error) {
	cfg := &graftOwnersConfig{
	...
	for lineNo := 1; scanner.Scan(); lineNo++ {
		...
		if len(fields) < 2 {
			return nil, fmt.Errorf(".graftowners:%d invalid rule: %q", lineNo, line)
		}
		if strings.EqualFold(fields[0], "team") {
			if len(fields) < 3 {
				return nil, fmt.Errorf(".graftowners:%d team rule requires members", lineNo)
			}
			teamName := normalizeOwnerName(fields[1])
			if teamName == "" {
				return nil, fmt.Errorf(".graftowners:%d invalid team name %q", lineNo, fields[1])
			}
			...
			for _, token := range fields[2:] {
				...
				if member == "" {
					return nil, fmt.Errorf(".graftowners:%d invalid team member %q", lineNo, token)
				}
				if strings.HasPrefix(member, "team/") {
					return nil, fmt.Errorf(".graftowners:%d teams cannot include other teams", lineNo)
				}
		...
		for _, token := range fields[1:] {
			...
			if err != nil {
				return nil, fmt.Errorf(".graftowners:%d %w", lineNo, err)
			}
...
func (c *graftOwnersConfig) ownersForChange(ch ownerEntityChange) []ownerRef {
	var owners []ownerRef
...
func (c *graftOwnersConfig) resolveOwners(owners []ownerRef) (resolvedUsers []string, unresolvedTeams []string) {
