...
import (
	...
	gotdiff "github.com/odvcencio/graft/pkg/diff"
...
func (s *PRService) evaluateEntityOwnerApprovals(ctx context.Context, repoID int64, pr *models.PullRequest, reviews []models.PRReview) ([]string, []EntityOwnerApprovalGate, error) {
	...
	cfg, err := loadGraftOwnersForBranch(store, pr.TargetBranch)
	if err != nil {
		return nil, nil, err
	}
	if len(cfg.rules) == 0 {
...
func (s *PRService) evaluateLintPass(ctx context.Context, repoID int64, pr *models.PullRequest) ([]string, error) {
	...
	tmpDir, err := os.MkdirTemp("", "orchard-pr-lint-*")
	if err != nil {
		return nil, err
	}
...
func loadGraftOwnersForBranch(store *gotstore.RepoStore, branch string) (*graftOwnersConfig, error) {
	...
	blobHash, err := findBlob(store.Objects, commit.TreeHash, ".graftowners")
	if err != nil {
		if isMissingFileErr(err) {
			return &graftOwnersConfig{teams: make(map[string][]string)}, nil
		}
		return nil, fmt.Errorf("read .graftowners: %w", err)
	}
	blob, err := store.Objects.ReadBlob(blobHash)
	if err != nil {
		return nil, fmt.Errorf("read .graftowners blob: %w", err)
	}
	return parseGraftOwners(blob.Data)
}
