...
import (
	...
	gitdiff "github.com/odvcencio/graft/pkg/diff"
...
func (s *WebhookService) deliverWithRetry(ctx context.Context, hook *models.Webhook, event string, body []byte, redeliveryOf *int64) (*models.WebhookDelivery, error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if err != nil {
			...
			req.Header.Set("User-Agent", "orchard-webhook/1.0")
			req.Header.Set("X-Orchard-Event", event)
			req.Header.Set("X-Orchard-Delivery", deliveryUID)
			if hook.Secret != "" {
...
func randomDeliveryUID() string {
	...
	if _, err := rand.Read(b[:]); err != nil {
		return fmt.Sprintf("orchard-%d", time.Now().UnixNano())
	}
